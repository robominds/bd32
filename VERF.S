* verf.s - orion Flash EEPROM verify driver
...
* do_verf does the comparision of the byte of data to memory

do_verf	move.l	#stack,a7			set up stack
...
* only run if do_verf returns non-zero (error); no need to check addresses
